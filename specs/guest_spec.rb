...
class TestGuest < MiniTest::Test
  ...
  def test_enough_cash?__true
    cost = 50
    assert_equal(true, @guest_1.enough_cash?(cost))
  end

  def test_enough_cash?__false
    cost = 100
    assert_equal(false, @guest_4.enough_cash?(cost))
  end

  def test_fav_song_on_list?__true
    assert_equal(true, @guest_1.fav_song_on_list(@room_1))
  end

  def test_fav_song_on_list__false
    assert_equal(false,@guest_3.fav_song_on_list(@room_1))
  end
